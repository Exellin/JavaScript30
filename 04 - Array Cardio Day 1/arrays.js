...
const fiftenHundredsInventors = inventors.filter(inventor => inventor.year >= 1500 && inventor.year < 1600);
console.table(fiftenHundredsInventors)
...
const ageSortedInventors = inventors.sort((a, b) => a.year > b.year ? 1 : -1);
...
const yearsLivedInventors = inventors.sort((a, b) => (a.passed - a.year) < (b.passed - b.year) ? 1 : -1);
...
const transportationCount = data.reduce((object, item) => {
  if (object[item]) {
    object[item] += 1;
  } else {
    object[item] = 1;
